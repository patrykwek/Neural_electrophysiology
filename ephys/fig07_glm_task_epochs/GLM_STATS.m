...
%
% ADDED (Option 1 implementation):
% - Compute a SESSION-LEVEL PSTH R^2 (one scalar per session) by:
%   (1) for each unit: compute observed vs predicted event-aligned PSTHs for cue/press/lick,
%       concatenate them (cue+press+lick), then store the vectors
%   (2) average observed PSTHs across units to get session PSTH_obs, same for PSTH_pred
%   (3) compute R^2 between session PSTH_obs and PSTH_pred (ignoring NaNs)
% - Save as GLM_OUTPUTS.sessR2_psth (no output filename changes)
...
% --- ADDED: session-level PSTH R^2 ---
sessR2_psth = nan(nSessions,1);
...
for sIdx = 1:nSessions
    ...
    % ---- determine which of the KEPT trials are "baseline-clean" ----
    ...
    Klick_units_cell  = {};
    R2_units_cell     = {};
    Baseline_units_cell = {}; % per-unit pre-cue baseline level

    % --- ADDED: per-unit concatenated PSTHs (observed & predicted) ---
    PSTHobs_units_cell  = {};
    PSTHpred_units_cell = {};
    ...
    for ch = 1:numel(spikes_by_ch)
        ...
        for u = 1:numel(uc)
            ...
            % ---- BASELINE subtraction using ONLY baseline-clean trials ----
            ...
            % ---- ADDED: per-unit concatenated PSTH vectors (obs & pred) ----
            [psthObsConcat, psthPredConcat] = psth_concat_from_session_y_( ...
                y, yhat, cueAbsK, pressAbsK, lickAbsK, tStartK, dt_ms, trialLensBins, trialOffsets, ...
                lags_cue_bins, lags_press_bins, lags_lick_bins);

            PSTHobs_units_cell{end+1,1}  = psthObsConcat;   %#ok<AGROW>
            PSTHpred_units_cell{end+1,1} = psthPredConcat;  %#ok<AGROW>
    ...
    % ---- ADDED: session-level PSTH R^2 (Option 1) ----
    PSTHobs_units  = cell2mat(PSTHobs_units_cell);   % [nUnits x nFeat]
    PSTHpred_units = cell2mat(PSTHpred_units_cell);  % [nUnits x nFeat]
    psthObs_sess   = mean(PSTHobs_units,  1, 'omitnan');
    psthPred_sess  = mean(PSTHpred_units, 1, 'omitnan');
    sessR2_psth(sIdx) = R2_nanmask_(psthObs_sess(:), psthPred_sess(:));

    nSessionsFitOK = nSessionsFitOK + 1;

    fprintf('  [ok] trials kept=%d | baseline-clean=%d | candidate units=%d | used units=%d | mean R2=%.3f | PSTH R2=%.3f | elapsed %.1fs\n', ...
        numel(idxKeep), nnz(isBaselineClean), sessCandidate, sessUsed, sessR2_full(sIdx), sessR2_psth(sIdx), toc(tSess));
end
...
GLM_OUTPUTS.sessR2_psth         = sessR2_psth;   % <-- ADDED
...
function r2 = R2_nanmask_(y, yhat)
    y = y(:); yhat = yhat(:);
    ...
    yh = yhat(m);
    ssr = sum((yy - yh).^2);
    sst = sum((yy - mean(yy)).^2) + eps;
    r2 = 1 - ssr/sst;
end

function [psthObsConcat, psthPredConcat] = psth_concat_from_session_y_( ...
    y, yhat, cueAbsK, pressAbsK, lickAbsK, tStartK, dt_ms, trialLensBins, trialOffsets, ...
    lags_cue_bins, lags_press_bins, lags_lick_bins)
    ...
    psthObs_cue   = event_psth_from_y_(y,    cueAbsK,   tStartK, dt_ms, trialLensBins, trialOffsets, lags_cue_bins);
    psthPred_cue  = event_psth_from_y_(yhat, cueAbsK,   tStartK, dt_ms, trialLensBins, trialOffsets, lags_cue_bins);

    psthObs_press  = event_psth_from_y_(y,    pressAbsK, tStartK, dt_ms, trialLensBins, trialOffsets, lags_press_bins);
    psthPred_press = event_psth_from_y_(yhat, pressAbsK, tStartK, dt_ms, trialLensBins, trialOffsets, lags_press_bins);

    psthObs_lick  = event_psth_from_y_(y,    lickAbsK,  tStartK, dt_ms, trialLensBins, trialOffsets, lags_lick_bins);
    psthPred_lick = event_psth_from_y_(yhat, lickAbsK,  tStartK, dt_ms, trialLensBins, trialOffsets, lags_lick_bins);

    % concatenate row vectors
    psthObsConcat  = [psthObs_cue(:)'  psthObs_press(:)'  psthObs_lick(:)'];
    psthPredConcat = [psthPred_cue(:)' psthPred_press(:)' psthPred_lick(:)'];

    % Ensure consistent length even if something odd happens
    if numel(psthObsConcat) ~= (numel(lags_cue_bins)+numel(lags_press_bins)+numel(lags_lick_bins))
        psthObsConcat  = nan(1, numel(lags_cue_bins)+numel(lags_press_bins)+numel(lags_lick_bins));
        psthPredConcat = nan(1, numel(lags_cue_bins)+numel(lags_press_bins)+numel(lags_lick_bins));
    end

    %#ok<NASGU> % nTr unused explicitly; kept for clarity
end

function psth = event_psth_from_y_(y, eventAbsK, tStartK, dt_ms, trialLensBins, trialOffsets, lags_bins)
    y = y(:);
    nTr = numel(eventAbsK);
    nL  = numel(lags_bins);
    ...
    for tr = 1:nTr
        ...
        evIdx = round((eventAbsK(tr) - ts)/dt_ms) + 1;
        evIdx = max(1, min(nT, evIdx));

        ti = evIdx + lags_bins(:)';  % 1 x nL
        keep = (ti >= 1) & (ti <= nT);

        if any(keep)
            M(tr, keep) = y(off + ti(keep));
        end
    end

    psth = mean(M, 1, 'omitnan');
end
