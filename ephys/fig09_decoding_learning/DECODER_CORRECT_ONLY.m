...
%   - For EACH SESSION:
%       * keep valid CORRECT trials with cue / press / lick / cueType / correct
...
%       * split CORRECT trials STRATIFIED within cue type: 75% train, 25% test
...
%       valid == true, correct == 1, RT >= MIN_RT_MS, cueType in {L,C,R}, press/lick inside fixedWin
%   - Unit inclusion uses classified-only and active-trial criterion within fixedWin
%   - Decoder is WITHIN SESSION (train 75% of CORRECT trials, test 25% of CORRECT trials)
...
MIN_RT_MS        = 100;
requireValid     = true;
minTrialsPerCue  = 2;    % minimum TOTAL correct trials per cue type in a session before split
...
for sIdx = 1:nSessions
    ...
    for k = 1:numel(trials)
        ...
        if any(~isfinite([cue press lick])), continue; end
        if ~isfinite(double(tr.correct)) || double(tr.correct) ~= 1, continue; end
    ...
    fprintf('  kept correct trials: total=%d | L=%d C=%d R=%d\n', numel(idxKeep), nL, nC, nR);

    if any([nL nC nR] < minTrialsPerCue)
        fprintf('  skipped: too few correct trials in at least one cue type.\n');
        continue;
    end

    % ---- first split only for summary printing ----
    [trainLocal0, testLocal0] = stratifiedSplitLCR_(cueTypeK, trainFrac);
    yTrain0 = categorical(cueTypeK(trainLocal0), ["L","C","R"]);
    yTest0  = categorical(cueTypeK(testLocal0),  ["L","C","R"]);

    if numel(categories(removecats(yTrain0))) < 3 || numel(categories(removecats(yTest0))) < 3
        fprintf('  skipped: split did not preserve all 3 cue classes in train/test.\n');
        continue;
    end

    nTrainUsed(sIdx) = numel(trainLocal0);
    nTestUsed(sIdx)  = numel(testLocal0);

    fprintf('  split: train=%d | test=%d | repeats=%d\n', numel(trainLocal0), numel(testLocal0), nSplitRepeats);
...
%% ---- GLOBAL DECODING SUMMARY STATS (ADDED) ----
sessMeanAcc_global = mean(AccMat, 1, 'omitnan');
sessMeanAcc_global_valid = sessMeanAcc_global(isfinite(sessMeanAcc_global));

if isempty(allVals)
    globalMean_allVals   = NaN;
    globalMedian_allVals = NaN;
    globalStd_allVals    = NaN;
    globalSEM_allVals    = NaN;
    globalVar_allVals    = NaN;
    globalIQR_allVals    = NaN;
    globalMin_allVals    = NaN;
    globalMax_allVals    = NaN;
    globalN_allVals      = 0;
else
    globalMean_allVals   = mean(allVals, 'omitnan');
    globalMedian_allVals = median(allVals, 'omitnan');
    globalStd_allVals    = std(allVals, 0, 'omitnan');
    globalSEM_allVals    = globalStd_allVals / sqrt(numel(allVals));
    globalVar_allVals    = var(allVals, 0, 'omitnan');
    globalIQR_allVals    = iqr(allVals);
    globalMin_allVals    = min(allVals);
    globalMax_allVals    = max(allVals);
    globalN_allVals      = numel(allVals);
end

if isempty(sessMeanAcc_global_valid)
    globalMean_sessMeans   = NaN;
    globalMedian_sessMeans = NaN;
    globalStd_sessMeans    = NaN;
    globalSEM_sessMeans    = NaN;
    globalVar_sessMeans    = NaN;
    globalIQR_sessMeans    = NaN;
    globalMin_sessMeans    = NaN;
    globalMax_sessMeans    = NaN;
    globalN_sessMeans      = 0;
else
    globalMean_sessMeans   = mean(sessMeanAcc_global_valid, 'omitnan');
    globalMedian_sessMeans = median(sessMeanAcc_global_valid, 'omitnan');
    globalStd_sessMeans    = std(sessMeanAcc_global_valid, 0, 'omitnan');
    globalSEM_sessMeans    = globalStd_sessMeans / sqrt(numel(sessMeanAcc_global_valid));
    globalVar_sessMeans    = var(sessMeanAcc_global_valid, 0, 'omitnan');
    globalIQR_sessMeans    = iqr(sessMeanAcc_global_valid);
    globalMin_sessMeans    = min(sessMeanAcc_global_valid);
    globalMax_sessMeans    = max(sessMeanAcc_global_valid);
    globalN_sessMeans      = numel(sessMeanAcc_global_valid);
end

fprintf('\n=== GLOBAL DECODING SUMMARY (ALL SESSION x BIN VALUES) ===\n');
fprintf('N      = %d\n', globalN_allVals);
fprintf('Mean   = %.6f\n', globalMean_allVals);
fprintf('Median = %.6f\n', globalMedian_allVals);
fprintf('Std    = %.6f\n', globalStd_allVals);
fprintf('SEM    = %.6f\n', globalSEM_allVals);
fprintf('Var    = %.6f\n', globalVar_allVals);
fprintf('IQR    = %.6f\n', globalIQR_allVals);
...
fprintf('\n=== GLOBAL DECODING SUMMARY (SESSION MEANS) ===\n');
fprintf('N      = %d\n', globalN_sessMeans);
fprintf('Mean   = %.6f\n', globalMean_sessMeans);
fprintf('Median = %.6f\n', globalMedian_sessMeans);
fprintf('Std    = %.6f\n', globalStd_sessMeans);
fprintf('SEM    = %.6f\n', globalSEM_sessMeans);
fprintf('Var    = %.6f\n', globalVar_sessMeans);
fprintf('IQR    = %.6f\n', globalIQR_sessMeans);
