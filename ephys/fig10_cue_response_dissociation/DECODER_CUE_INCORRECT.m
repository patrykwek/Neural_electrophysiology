%% ========= FIG3X_SESSION_CUETYPE_DECODING_HEATMAP__TRAINCORRECT_TESTINCORRECT__A_THESIS_FINAL__GLOBALWARP__CLASSIFIEDONLY.m =========
...
%       * split ALL kept trials STRATIFIED within cue type: 75% train, 25% held-out
%       * train decoder on the ALL-TRIAL train split
%       * TEST decoder on INCORRECT trials from the held-out split only
%       * predict cueType on incorrect held-out trials
...
%       valid == true, RT >= MIN_RT_MS, cueType in {L,C,R}, correct in {0,1},
...
%   - Removed the requirement to reserve one incorrect held-out trial per cue class
...
outDir  = fullfile(baseOut, 'DECODING_CUETYPE_HEATMAP_TRAINCORRECT_TESTINCORRECT');
...
%% ---- BARPLOT OUTPUTS ----
outBarMeanPng  = fullfile(outDir, 'CUETYPE_DECODING__TRAINCORRECT_TESTINCORRECT__EARLY_LATE_BAR__MEANACC.png');
outBarMeanSvg  = fullfile(outDir, 'CUETYPE_DECODING__TRAINCORRECT_TESTINCORRECT__EARLY_LATE_BAR__MEANACC.svg');
outBarMeanFig  = fullfile(outDir, 'CUETYPE_DECODING__TRAINCORRECT_TESTINCORRECT__EARLY_LATE_BAR__MEANACC.fig');

outCueBarPng   = fullfile(outDir, 'CUETYPE_DECODING__TRAINCORRECT_TESTINCORRECT__EARLY_LATE_BAR__CueSegment.png');
outCueBarSvg   = fullfile(outDir, 'CUETYPE_DECODING__TRAINCORRECT_TESTINCORRECT__EARLY_LATE_BAR__CueSegment.svg');
outCueBarFig   = fullfile(outDir, 'CUETYPE_DECODING__TRAINCORRECT_TESTINCORRECT__EARLY_LATE_BAR__CueSegment.fig');

outPressBarPng = fullfile(outDir, 'CUETYPE_DECODING__TRAINCORRECT_TESTINCORRECT__EARLY_LATE_BAR__PressSegment.png');
outPressBarSvg = fullfile(outDir, 'CUETYPE_DECODING__TRAINCORRECT_TESTINCORRECT__EARLY_LATE_BAR__PressSegment.svg');
outPressBarFig = fullfile(outDir, 'CUETYPE_DECODING__TRAINCORRECT_TESTINCORRECT__EARLY_LATE_BAR__PressSegment.fig');

outLickBarPng  = fullfile(outDir, 'CUETYPE_DECODING__TRAINCORRECT_TESTINCORRECT__EARLY_LATE_BAR__LickSegment.png');
outLickBarSvg  = fullfile(outDir, 'CUETYPE_DECODING__TRAINCORRECT_TESTINCORRECT__EARLY_LATE_BAR__LickSegment.svg');
outLickBarFig  = fullfile(outDir, 'CUETYPE_DECODING__TRAINCORRECT_TESTINCORRECT__EARLY_LATE_BAR__LickSegment.fig');
...
fprintf('\n=== SESSION-WISE CUE-TYPE DECODING (TRAIN ALL SPLIT / TEST HELD-OUT INCORRECT) ===\n');
...
for sIdx = 1:nSessions
    ...
    trainLabels = cueTypeK(trainLocal);
    testLabels  = cueTypeK(testLocal);
    ...
    if any([nL_te nC_te nR_te] < minTrialsPerCue)
        fprintf('  skipped: too few held-out incorrect trials in at least one cue type for testing.\n');
        continue;
    end
    ...
    for ch = 1:numel(spikes_by_ch)
        ...
        for u = 1:numel(uc)
            ...
            % require all train/test trials to be finite for this unit
...
title(ax, 'Cue-type decoding on incorrect trials (train all split, test incorrect)', ...
...
%% ---- SAVE HEATMAP ----
outbase = fullfile(outDir, sprintf('Sessions01_%02d_CueTypeDecodingHeatmap_TRAINCORRECT_TESTINCORRECT_GLOBALWARP_BIN%03dms', ...
    nSessions, binSizeMs));
...
title(ax2, 'Cue decoding on incorrect trials: Early vs Late', ...
    'FontWeight','bold', 'FontSize', titleFontSize);
...
outbaseTrace = fullfile(outDir, sprintf('Sessions01_%02d_CueTypeDecodingTrace_TRAINCORRECT_TESTINCORRECT_CLUSTERPERM_GLOBALWARP_BIN%03dms', ...
    nSessions, binSizeMs));
...
figBarMean = plotGroupBar_perm_(earlyMeanAcc, lateMeanAcc, colEarlyShade, colLateShade, ...
    'Accuracy', 'Mean incorrect-trial accuracy (cue decoder)', ...
...
figCueBar = plotGroupBar_perm_(earlyCueAcc, lateCueAcc, colEarlyShade, colLateShade, ...
    'Accuracy', 'Cue-segment incorrect-trial accuracy (cue decoder)', ...
    labelFontSize, titleFontSize, tickFontSize, axesTickLW, ...
    scatterMS, scatterEdgeLW, jit, greyDot, Nperm_bar);
...
figPressBar = plotGroupBar_perm_(earlyPressAcc, latePressAcc, colEarlyShade, colLateShade, ...
    'Accuracy', 'Press-segment incorrect-trial accuracy (cue decoder)', ...
    labelFontSize, titleFontSize, tickFontSize, axesTickLW, ...
    scatterMS, scatterEdgeLW, jit, greyDot, Nperm_bar);
...
figLickBar = plotGroupBar_perm_(earlyLickAcc, lateLickAcc, colEarlyShade, colLateShade, ...
    'Accuracy', 'Lick-segment incorrect-trial accuracy (cue decoder)', ...
    labelFontSize, titleFontSize, tickFontSize, axesTickLW, ...
    scatterMS, scatterEdgeLW, jit, greyDot, Nperm_bar);
...
outMat = fullfile(outDir, sprintf('Sessions01_%02d_CueTypeDecodingResults_TRAINCORRECT_TESTINCORRECT_GLOBALWARP_BIN%03dms.mat', ...
    nSessions, binSizeMs));
