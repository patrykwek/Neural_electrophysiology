%% ========= FIG3X_SESSION_CUETYPE_DECODING_HEATMAP__TRAINALL_TESTINCORRECT__A_THESIS_FINAL__GLOBALWARP__CLASSIFIEDONLY.m =========
...
%   - For EACH SESSION:
%       * keep valid trials with cue / press / lick / cueType / correct
...
%       * repeatedly split ALL kept trials STRATIFIED within cue type: trainFrac train, held-out remainder
%       * train a within-session decoder on the ALL-TRIAL train split
%       * TEST decoder on INCORRECT trials from the held-out split only
%       * if a held-out split contains no incorrect trials, fall back to all incorrect trials as test
%       * store held-out incorrect-trial decoding ACCURACY for every time bin
%
% OUTPUT
%   - One heatmap:
%       y-axis = session
%       x-axis = warped time bins
%       color  = incorrect-trial cue-type decoding accuracy in that session/time bin
...
%   - One saved MAT with:
%       * decoding matrix and warped-time axis
%       * global warp targets and segment labels
%       * session-level mean / interval accuracies
%       * train/test counts and per-session bookkeeping
%       * settings / metadata needed for later comparison to press decoding
%
% STYLE
...
%
% NOTES
%   - Global warp targets are behavior-only and pooled across all sessions
%   - Session trial inclusion for decoding uses:
%       valid == true, correct in {0,1}, RT >= MIN_RT_MS, cueType in {L,C,R},
...
%
% NEW CHANGE
%   - Repeat the stratified train/test split many times and average decoding accuracy across repeats
%   - Train on ALL kept trials; test on INCORRECT only
%   - If a held-out split contains no incorrect trials, fall back to all incorrect trials as test
...
%   - Save decoding results and all later-comparison specifications to .mat
...
outDir  = fullfile(baseOut, 'DECODING_CUETYPE_HEATMAP_TRAINALL_TESTINCORRECT');
...
MIN_RT_MS        = 200;
requireValid     = true;
minTrialsPerCue  = 0;    % minimum incorrect test trials per cue type after split (0 = allow sparse incorrect test)
...
doZscore       = true;
rngSeed        = 0;
nSplitRepeats  = 50;     % repeated stratified splits per session
...
%% ---- BARPLOT OUTPUTS ----
outBarMeanPng  = fullfile(outDir, 'CUETYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__MEANACC.png');
outBarMeanSvg  = fullfile(outDir, 'CUETYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__MEANACC.svg');
outBarMeanFig  = fullfile(outDir, 'CUETYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__MEANACC.fig');

outCueBarPng   = fullfile(outDir, 'CUETYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__CueSegment.png');
outCueBarSvg   = fullfile(outDir, 'CUETYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__CueSegment.svg');
outCueBarFig   = fullfile(outDir, 'CUETYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__CueSegment.fig');

outPressBarPng = fullfile(outDir, 'CUETYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__PressSegment.png');
outPressBarSvg = fullfile(outDir, 'CUETYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__PressSegment.svg');
outPressBarFig = fullfile(outDir, 'CUETYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__PressSegment.fig');

outLickBarPng  = fullfile(outDir, 'CUETYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__LickSegment.png');
outLickBarSvg  = fullfile(outDir, 'CUETYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__LickSegment.svg');
outLickBarFig  = fullfile(outDir, 'CUETYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__LickSegment.fig');
...
nTrainUsed         = nan(nSessions,1);   % train trials used for first valid split summary
nTestUsed          = nan(nSessions,1);   % incorrect test trials used for first valid split summary
nAllTrialsUsed     = nan(nSessions,1);
nIncorrectAvail    = nan(nSessions,1);
nIncorrectUsedMean = nan(nSessions,1);
nFallbackRepeats   = nan(nSessions,1);

fprintf('\n=== SESSION-WISE CUE-TYPE DECODING (TRAIN ALL / TEST INCORRECT) ===\n');
...
for sIdx = 1:nSessions
    ...
    for k = 1:numel(trials)
        ...
        if ~all(isfield(tr, {'cue','press','lick','cueType','correct'})), continue; end
        ...
        if ~isfinite(corrVal) || ~(corrVal == 0 || corrVal == 1), continue; end
    ...
    nAllTrialsUsed(sIdx)  = numel(idxKeep);
    nIncorrectAvail(sIdx) = numel(idxIncorrect);
    ...
    if any([nL_all nC_all nR_all] < 2)
        fprintf('  skipped: too few total trials in at least one cue type for stratified split.\n');
        continue;
    end
    ...
    for ch = 1:numel(spikes_by_ch)
        ...
        for u = 1:numel(uc)
            ...
            % require all kept trials to be finite for this unit
            if any(~all(isfinite(Xunit),2))
                continue;
            end
    ...
    % ---- decode cue type separately for each time bin, averaged across repeated splits ----
    accRep = nan(nSplitRepeats, nBins);
    testCountRep = nan(nSplitRepeats,1);
    fallbackUsedRep = false(nSplitRepeats,1);

    firstGoodSummaryCaptured = false;

    for iRep = 1:nSplitRepeats
        [trainLocal_all, testLocal_all] = stratifiedSplitLCR_(cueTypeK, trainFrac);

        % test on incorrect trials from held-out split only
        trainLocal = trainLocal_all(:);
        testLocal  = intersect(testLocal_all(:), idxIncorrect(:), 'stable');

        % fallback if held-out split has no incorrect trials
        if isempty(testLocal)
            fallbackUsedRep(iRep) = true;
        ...
        nL_te = nnz(cueTypeK(testLocal) == "L");
        nC_te = nnz(cueTypeK(testLocal) == "C");
        nR_te = nnz(cueTypeK(testLocal) == "R");
        if any([nL_te nC_te nR_te] < minTrialsPerCue)
            continue;
        end

        testCountRep(iRep) = numel(testLocal);

        if ~firstGoodSummaryCaptured
            nTrainUsed(sIdx) = numel(trainLocal);
            nTestUsed(sIdx)  = numel(testLocal);
            firstGoodSummaryCaptured = true;
            fprintf('  first valid split: train(all)=%d | test(incorrect)=%d | repeats=%d\n', ...
                numel(trainLocal), numel(testLocal), nSplitRepeats);
        end
        ...
        for b = 1:nBins
            ...
            try
                ...
                accRep(iRep, b) = mean(yHat(ok) == yTe(ok));
            catch ME
                warning('Session %02d repeat %d bin %d decode failed: %s', sIdx, iRep, b, ME.message);
                accRep(iRep, b) = nan;
            end
        end
    end

    AccMat(:, sIdx) = mean(accRep, 1, 'omitnan')';
    nIncorrectUsedMean(sIdx) = mean(testCountRep, 'omitnan');
    nFallbackRepeats(sIdx)   = nnz(fallbackUsedRep);

    fprintf('  decoded bins: %d/%d | mean incorrect test trials per repeat: %.2f | fallback repeats: %d/%d\n', ...
        nnz(isfinite(AccMat(:,sIdx))), nBins, nIncorrectUsedMean(sIdx), nFallbackRepeats(sIdx), nSplitRepeats);
end
...
title(ax, 'Cue-type decoding on incorrect trials (train all, test incorrect)', ...
...
%% ---- SAVE HEATMAP ----
outbase = fullfile(outDir, sprintf('Sessions01_%02d_CueTypeDecodingHeatmap_TRAINALL_TESTINCORRECT_GLOBALWARP_BIN%03dms_train%02d_repeats%03d', ...
    nSessions, binSizeMs, round(100*trainFrac), nSplitRepeats));
...
title(ax2, 'Cue-type decoding on incorrect trials: Early vs Late', ...
    'FontWeight','bold', 'FontSize', titleFontSize);
...
outbaseTrace = fullfile(outDir, sprintf('Sessions01_%02d_CueTypeDecodingTrace_TRAINALL_TESTINCORRECT_CLUSTERPERM_GLOBALWARP_BIN%03dms_train%02d_repeats%03d', ...
    nSessions, binSizeMs, round(100*trainFrac), nSplitRepeats));
...
figBarMean = plotGroupBar_perm_(earlyMeanAcc, lateMeanAcc, colEarlyShade, colLateShade, ...
    'Accuracy', 'Mean incorrect-trial cue-type decoding accuracy', ...
...
figCueBar = plotGroupBar_perm_(earlyCueAcc, lateCueAcc, colEarlyShade, colLateShade, ...
    'Accuracy', 'Cue-segment incorrect-trial cue-type decoding accuracy', ...
    labelFontSize, titleFontSize, tickFontSize, axesTickLW, ...
    scatterMS, scatterEdgeLW, jit, greyDot, Nperm_bar);
...
figPressBar = plotGroupBar_perm_(earlyPressAcc, latePressAcc, colEarlyShade, colLateShade, ...
    'Accuracy', 'Press-segment incorrect-trial cue-type decoding accuracy', ...
    labelFontSize, titleFontSize, tickFontSize, axesTickLW, ...
    scatterMS, scatterEdgeLW, jit, greyDot, Nperm_bar);
...
figLickBar = plotGroupBar_perm_(earlyLickAcc, lateLickAcc, colEarlyShade, colLateShade, ...
    'Accuracy', 'Lick-segment incorrect-trial cue-type decoding accuracy', ...
    labelFontSize, titleFontSize, tickFontSize, axesTickLW, ...
    scatterMS, scatterEdgeLW, jit, greyDot, Nperm_bar);
...
outMat = fullfile(outDir, sprintf('Sessions01_%02d_CueTypeDecodingResults_TRAINALL_TESTINCORRECT_GLOBALWARP_BIN%03dms_train%02d_repeats%03d.mat', ...
    nSessions, binSizeMs, round(100*trainFrac), nSplitRepeats));

decodeMode = 'TRAIN_ALL_TEST_INCORRECT';
decoderName = 'fitcecoc_linear_logistic_ridge';
comparisonReadyFields = { ...
    'AccMat','binCenters_ms','xPlot','segLabels','cueMask','pressMask','lickMask', ...
    'sessMeanAcc','sessCueAcc','sessPressAcc','sessLickAcc', ...
    'earlySess','lateSess','Tpress','Tlick','cueLine','pressLine','lickLine', ...
    'binSizeMs','trainFrac','nSplitRepeats','MIN_RT_MS','fixedWin', ...
    'nUnitsUsed','nTrainUsed','nTestUsed','nAllTrialsUsed','nIncorrectAvail', ...
    'nIncorrectUsedMean','nFallbackRepeats','decodeMode','decoderName'};
...
for sIdx = 1:nSessions
    ...
    fprintf('Session %02d: units=%g train_all=%g test_incorrect=%g all_kept=%g incorrect_avail=%g incorrect_mean_test=%.2f fallback_repeats=%g decoded_bins=%d/%d\n', ...
        sIdx, nUnitsUsed(sIdx), nTrainUsed(sIdx), nTestUsed(sIdx), ...
        nAllTrialsUsed(sIdx), nIncorrectAvail(sIdx), nIncorrectUsedMean(sIdx), ...
        nFallbackRepeats(sIdx), nGoodBins, nBins);
end
...
function [trainIdx, testIdx] = stratifiedSplitLCR_(cueTypeK, trainFrac)
    ...
    for i = 1:numel(cats)
        ...
        nTrain = min(nTrain, numel(idx)-1);  % preserve at least one held-out trial if possible
