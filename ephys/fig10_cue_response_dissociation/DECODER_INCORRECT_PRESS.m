%% ========= FIG3X_SESSION_PRESSTYPE_DECODING_HEATMAP__TRAINALL_TESTINCORRECT__A_THESIS_FINAL__GLOBALWARP__CLASSIFIEDONLY.m =========
...
%       * repeatedly split ALL kept trials STRATIFIED within press type: trainFrac train, held-out remainder
...
%       color  = incorrect-trial press-type decoding accuracy in that session/time bin
...
%       * settings / metadata needed for later comparison to cue decoding
%
% STYLE
...
%       valid == true, correct in {0,1}, RT >= MIN_RT_MS, pokeType in {L,C,R},
...
outDir  = fullfile(baseOut, 'DECODING_PRESSTYPE_HEATMAP_TRAINALL_TESTINCORRECT');
...
MIN_RT_MS        = 200;
requireValid     = true;
minTrialsPerCue  = 0;    % minimum incorrect test trials per press type after split (0 = allow sparse incorrect test)
...
%% ---- BARPLOT OUTPUTS ----
outBarMeanPng  = fullfile(outDir, 'PRESSTYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__MEANACC.png');
outBarMeanSvg  = fullfile(outDir, 'PRESSTYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__MEANACC.svg');
outBarMeanFig  = fullfile(outDir, 'PRESSTYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__MEANACC.fig');

outCueBarPng   = fullfile(outDir, 'PRESSTYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__CueSegment.png');
outCueBarSvg   = fullfile(outDir, 'PRESSTYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__CueSegment.svg');
outCueBarFig   = fullfile(outDir, 'PRESSTYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__CueSegment.fig');

outPressBarPng = fullfile(outDir, 'PRESSTYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__PressSegment.png');
outPressBarSvg = fullfile(outDir, 'PRESSTYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__PressSegment.svg');
outPressBarFig = fullfile(outDir, 'PRESSTYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__PressSegment.fig');

outLickBarPng  = fullfile(outDir, 'PRESSTYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__LickSegment.png');
outLickBarSvg  = fullfile(outDir, 'PRESSTYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__LickSegment.svg');
outLickBarFig  = fullfile(outDir, 'PRESSTYPE_DECODING__TRAINALL_TESTINCORRECT__EARLY_LATE_BAR__LickSegment.fig');
...
fprintf('\n=== SESSION-WISE PRESS-TYPE DECODING (TRAIN ALL / TEST INCORRECT) ===\n');
...
for sIdx = 1:nSessions
    ...
    % ---- decode press type separately for each time bin, averaged across repeated splits ----
    ...
    for iRep = 1:nSplitRepeats
        ...
        nL_te = nnz(pressTypeK(testLocal) == "L");
        nC_te = nnz(pressTypeK(testLocal) == "C");
        nR_te = nnz(pressTypeK(testLocal) == "R");
...
title(ax, 'Press-type decoding on incorrect trials (train all, test incorrect)', ...
...
%% ---- SAVE HEATMAP ----
outbase = fullfile(outDir, sprintf('Sessions01_%02d_PressTypeDecodingHeatmap_TRAINALL_TESTINCORRECT_GLOBALWARP_BIN%03dms_train%02d_repeats%03d', ...
...
title(ax2, 'Press-type decoding on incorrect trials: Early vs Late', ...
    'FontWeight','bold', 'FontSize', titleFontSize);
...
outbaseTrace = fullfile(outDir, sprintf('Sessions01_%02d_PressTypeDecodingTrace_TRAINALL_TESTINCORRECT_CLUSTERPERM_GLOBALWARP_BIN%03dms_train%02d_repeats%03d', ...
    nSessions, binSizeMs, round(100*trainFrac), nSplitRepeats));
...
figBarMean = plotGroupBar_perm_(earlyMeanAcc, lateMeanAcc, colEarlyShade, colLateShade, ...
    'Accuracy', 'Mean incorrect-trial press-type decoding accuracy', ...
...
figCueBar = plotGroupBar_perm_(earlyCueAcc, lateCueAcc, colEarlyShade, colLateShade, ...
    'Accuracy', 'Cue-segment incorrect-trial press-type decoding accuracy', ...
    labelFontSize, titleFontSize, tickFontSize, axesTickLW, ...
    scatterMS, scatterEdgeLW, jit, greyDot, Nperm_bar);
...
figPressBar = plotGroupBar_perm_(earlyPressAcc, latePressAcc, colEarlyShade, colLateShade, ...
    'Accuracy', 'Press-segment incorrect-trial press-type decoding accuracy', ...
    labelFontSize, titleFontSize, tickFontSize, axesTickLW, ...
    scatterMS, scatterEdgeLW, jit, greyDot, Nperm_bar);
...
figLickBar = plotGroupBar_perm_(earlyLickAcc, lateLickAcc, colEarlyShade, colLateShade, ...
    'Accuracy', 'Lick-segment incorrect-trial press-type decoding accuracy', ...
    labelFontSize, titleFontSize, tickFontSize, axesTickLW, ...
    scatterMS, scatterEdgeLW, jit, greyDot, Nperm_bar);
...
outMat = fullfile(outDir, sprintf('Sessions01_%02d_PressTypeDecodingResults_TRAINALL_TESTINCORRECT_GLOBALWARP_BIN%03dms_train%02d_repeats%03d.mat', ...
    nSessions, binSizeMs, round(100*trainFrac), nSplitRepeats));
