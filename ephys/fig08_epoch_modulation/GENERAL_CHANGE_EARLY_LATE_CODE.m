...
% for computing mean(|Z|). Windows are applied per-trial around each event time (Option A).
%
...
outPng  = fullfile(baseOut, '3_GENERAL_CHANGE_EARLY_LATE.png'); % (name kept as requested)
...
% Returns: one row per SESSION, columns are [cue press lick] epoch metrics.
...
function M = computeAbsZMetricsForSessions_(sessRange, beh, S, cell_type, ...
    ...
    % SESSION-LEVEL output: one row per session
    ...
    for sIdx = s0:s1
        ...
        for k = 1:numel(trials)
            ...
            keepTrial(k) = true;
            cueAbs(k)    = cue;
            pressLat(k)  = rt;   % RT (ms from cue)
            lickLat(k)   = lr;   % LR (ms from cue)
        end
        ...
        % Collect per-UNIT metrics for this session, then average -> session value
        M_units = nan(0,3); % nUnitsUsed x 3
        ...
        for ch = 1:numel(spikes_by_ch)
            ...
            for u = 1:numel(uc)
                ...
                % Per-trial window metrics, then average -> per-unit
                m1_tr = nan(nTrU,1);
                m2_tr = nan(nTrU,1);
                m3_tr = nan(nTrU,1);
                ...
                for iTr = 1:nTrU
                    cue0   = cueU(iTr);
                    tPress = rtU(iTr);   % ms from cue
                    tLick  = lkU(iTr);   % ms from cue

                    % ---- SDF (Hz) on the common cue-aligned axis [winLeft..winRight] ----
                    ...
                    sm = conv(counts, g, 'same');     % spikes/bin
                    y  = sm / (dt_ms/1000);           % Hz

                    z = zscoreTrial_(y);

                    % ---- Convert per-trial event windows to indices on this axis ----
                    ...
                    % ---- Metrics: mean(|Z|) inside each window on this trial ----
                    m1_tr(iTr) = mean(abs(z(idxCue(1):idxCue(2))),     'omitnan');
                    m2_tr(iTr) = mean(abs(z(idxPress(1):idxPress(2))), 'omitnan');
                    m3_tr(iTr) = mean(abs(z(idxLick(1):idxLick(2))),   'omitnan');
                end
        ...
        % SESSION metric = mean across units in this session
...
function idx12 = msWindowToIdx_(w_ms, winLeft_ms, dt_ms, nT)
    % Convert [tStart_ms, tEnd_ms] to [idxStart, idxEnd] (clamped, sorted)
